  ...
  constructor(server: Server<DefaultEventsMap, DefaultEventsMap, DefaultEventsMap, any>) {
    ...
    this.server.on('connection', (socket: any ) => {
      ...
      socket.on('rca_connection', (auth_code: string) => {
        ...
        if (socket.id) this.rcas_codes.set(auth_code, socket.id)
      })
      ...
      socket.on('ctrlr_connection_request', (auth_code: string) => {
        ...
        if (!this.rcas_codes.get(auth_code)) {
          ...
          if (socket.id) codeCtrlrs.concat([socket.id])
      ...
      socket.on('accept_controller_response', (ctrlr_socket_id: string, response: boolean, temp_id: string) => {
        console.log('RESPONSE', response)
        if (response == true) {
          this.ctrlrs_ids.set(ctrlr_socket_id, temp_id)
          if (socket.id) this.ctrlrs_rcas.set(ctrlr_socket_id, socket.id)
...
export abstract class RCA {
  ...
  constructor(serverAddress: string, code: string) {
    ...
    this.socket.on('control', (ctrlr_id: string, action_info: any) => {
      ...
      this.onControl(ctrlr_id, action_info)
    })

  }
  public abstract onControl(source_temp_id: string, action_info: any): void
